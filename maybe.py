...
def main():
    ...
    while instruction_queue:
        ...
        elif current_instruction.operation == "MUL" and mul_station.is_available():
            Vj, Vk, Qj, Qk = resolve_operands(register_file, current_instruction.operands)
            add_station.allocate(current_instruction.operation, Vj=Vj, Vk=Vk, Qj=Qj, Qk=Qk)
            
        elif current_instruction.operation == "CALL" and call_ret_station.is_available():
            # For CALL/RET, resolve only the necessary operands or use PC directly
            Vj, _, _, _ = resolve_operands(register_file, current_instruction.operands)
            call_ret_station.allocate(current_instruction.operation, Vj=Vj)
            print(f"Cycle {clock_cycles}: Allocating CALL/RET - Vj: {Vj}")
        ...
        mul_station.execute(register_file, memory)
        call_ret_station.execute(register_file, memory)
        ...
        ipc = (completed_instructions / total_instructions) * 100
        print(f"IPC (Instruction per Cycle): {ipc:.2f}%")
