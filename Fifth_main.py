...
def execute_and_commit(reservation_station, register_file, reorder_buffer, current_cycle, metadata):
    ...
    if completed_station:
        # Add start_exec to metadata when the instruction starts execution
        for entry in metadata:
            if entry['instruction'] == completed_station['op'] and entry.get('start_exec') is None:
                entry['start_exec'] = current_cycle  # Set start execution time
                break
